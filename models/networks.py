...
def define_network(input_nc, lstm_hidden_size, model, init_from=None, isTest=False, gpu_ids=[]):
    ...
    # For KD model
    elif model == 'Student' or model == "onlyStudent":
        netG = Student(input_nc, weights=init_from, isTest=isTest, gpu_ids=gpu_ids)
    ...
    def __init__(self, lossID, weights=None, lstm_hidden_size=None):
        ...
        if lossID != "loss3":
            self.projection = nn.Sequential(*[nn.AvgPool2d(kernel_size=5, stride=3),
                                              weight_init_googlenet(lossID + "/conv",
                                                                    nn.Conv2d(nc_loss[lossID], 128, kernel_size=1),
                                                                    weights),
            ...
            if lstm_hidden_size is not None:
                self.lstm_pose_lr = weight_init_googlenet("LSTM", nn.LSTM(input_size=32, hidden_size=lstm_hidden_size,
                                                                          bidirectional=True, batch_first=True))
                self.lstm_pose_ud = weight_init_googlenet("LSTM", nn.LSTM(input_size=32, hidden_size=lstm_hidden_size,
                                                                          bidirectional=True, batch_first=True))
        else:
            ...
            if lstm_hidden_size is not None:
                self.lstm_pose_lr = weight_init_googlenet("LSTM", nn.LSTM(input_size=64, hidden_size=lstm_hidden_size,
                                                                          bidirectional=True, batch_first=True))
                self.lstm_pose_ud = weight_init_googlenet("LSTM", nn.LSTM(input_size=32, hidden_size=lstm_hidden_size,
                                                                          bidirectional=True, batch_first=True))
...
class InceptionBlock(nn.Module):
    def __init__(self, incp, input_nc, x1_nc, x3_reduce_nc, x3_nc, x5_reduce_nc,
        ...
        self.branch_x3 = nn.Sequential(*[
            weight_init_googlenet("inception_" + incp + "/3x3_reduce", nn.Conv2d(input_nc, x3_reduce_nc, kernel_size=1),
                                  weights),
            nn.ReLU(inplace=True),
            weight_init_googlenet("inception_" + incp + "/3x3",
                                  nn.Conv2d(x3_reduce_nc, x3_nc, kernel_size=3, padding=1), weights),
            nn.ReLU(inplace=True)])

        self.branch_x5 = nn.Sequential(*[
            weight_init_googlenet("inception_" + incp + "/5x5_reduce", nn.Conv2d(input_nc, x5_reduce_nc, kernel_size=1),
                                  weights),
            nn.ReLU(inplace=True),
        ...
        self.branch_proj = nn.Sequential(*[
            ...
            weight_init_googlenet("inception_" + incp + "/pool_proj", nn.Conv2d(input_nc, proj_nc, kernel_size=1),
                                  weights),
            nn.ReLU(inplace=True)])
...
class PoseNet(nn.Module):
    def __init__(self, input_nc, weights=None, isTest=False, gpu_ids=[]):
        ...
        self.before_inception_1 = nn.Sequential(*[
        ...
        self.before_inception_2 = nn.Sequential(*[
    ...
    def forward(self, input):
        ...
        output_bf_1 = self.before_inception_1(input)
        output_bf_2 = self.before_inception_2(output_bf_1)
        output_3a = self.inception_3a(output_bf_2)
        ...
        # *** Student 학습시킬 때
        # pred_T, feature_hint, feature_T 순서
        # feature_T => output_bf_1, output_bf_2, output_3a 등으로 테스트
        return self.cls3_fc(output_5b), output_5b, output_bf_1
...
class Student(nn.Module):
    def __init__(self, input_nc, weights=None, isTest=False, gpu_ids=[]):
        ...
        self.before_cls1 = nn.Sequential(*[
            weight_init_googlenet("conv", nn.Conv2d(512, 1024, kernel_size=2, stride=2, padding=0)),
            nn.ReLU(inplace=True)])
        self.cls1_fc = RegressionHead(lossID="loss3", weights=weights)

        self.model = nn.Sequential(*[self.inception_3a, self.inception_3b,
                                     self.inception_4a,
                                     self.before_cls1, self.cls1_fc,
                                     ])
        if self.isTest:
            self.model.eval()  # ensure Dropout is deactivated during test
    ...
    def forward(self, input):
        ...
        feature_guided = self.before_cls1(output_4a)
        pred_S = self.cls1_fc(feature_guided)

        # pred_S, feature_guided, feature_S
        # feature_T => output_bf_1, output_bf_2, output_3a 등으로 테스트
        if not self.isTest:
            return pred_S, feature_guided, output_bf_1
