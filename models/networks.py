...
class PoseNet(nn.Module):
    ...
    def forward(self, input):
        ...
        return self.cls3_fc(output_5b), output_5b
...
class Student(nn.Module):
    def __init__(self, input_nc, weights=None, isTest=False,  gpu_ids=[]):
        ...
        self.featuremap_1 = nn.Sequential(*[
        ...
        self.model = nn.Sequential(*[self.inception_3a, self.inception_3b,
                                   self.inception_4a,
                                   self.featuremap_1, self.cls1_fc,
                                   ])
        if self.isTest:
            self.model.eval() # ensure Dropout is deactivated during test
    ...
    def forward(self, input):
        ...
        out_1 = self.cls1_fc(output_4a) # [pos, ori]
        out_feature1 = self.featuremap_1(output_4a)

        if not self.isTest :
            return out_1, out_feature1

        return out_1

class Student_cls1(nn.Module):
    def __init__(self, input_nc, weights=None, isTest=False,  gpu_ids=[]):
        super(Student_cls1, self).__init__()
        self.gpu_ids = gpu_ids
        self.isTest = isTest
        self.before_inception = nn.Sequential(*[
            weight_init_googlenet("conv1/7x7_s2", nn.Conv2d(input_nc, 64, kernel_size=7, stride=2, padding=3), weights),
